...
\hypersetup{
  ...
  pdfsubject={Research Scientist at the intersection of AI and Physics},
...
% template, so `` is a literal keyword here (empty for the
% generic CV, which prints everything).
\defbibfilter{cvicml}{ \keyword{icml2026} }
\defbibfilter{cvselected}{ not \keyword{icml2026} }
...
\begin{itemize}[leftmargin=\projbulletindent]
  \item Built a GAN for high-fidelity, tabular-data simulation of a Cherenkov detector
  \item Reached ~10⁵× speed-up over the full Geant4 simulation
\end{itemize}
\vspace{2pt}
...
\section{Skills}
\begin{itemize}
  \item \textbf{Generative modelling for scientific data} --- GANs, autoregressive transformers, and diffusion; from fast detector simulation to symmetry-aware crystal generation.
  ...
  \item \textbf{Physics domain depth} --- high-energy and condensed-matter physics; first-principles intuition that shapes model design.
